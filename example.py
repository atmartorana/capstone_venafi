...
def passPolicyEnvoy():
    old = "example.rego"
    new = "/home/venafi_cmu/Spire-kubernetes/k8s/envoy-opa/k8s/backend/config/opa-policy.rego"
    # new = "./opa-policy.rego"
    try:
        subprocess.run("mv %s %s" %(old, new), shell=True, check=True)
        ...
        print("[** Pushed example.rego file to envoy proxy and renamed opa-policy.rego**]")
    except:
        print("[** Unable to move file to correct directory (/home/venafi_cmu/Spire-kubernetes/k8s/envoy-opa/k8s/backend/config/opa-policy.rego) **]")
...
try:
    if __name__ == "__main__":
        ...
        if context == "authorization":
            ...
            print("[** Context of %s is not supported **]" %(context))
